...
def mistakes(choices, data, target):
    mistakes = 0

    for i in range(len(data)):
        print "Real: " + data[i][target] + ", " + "guessed: " + choices[i]

        if choices[i] != data[i][target]:
            mistakes += 1
...
def vote (forest, data):
    """The random forest equivalent of classify."""
    ...
    for record in data:
        ...
        for tree in forest:
            votes.append(get_classification(record, tree))
        ...
        for option in count:
            if count[option] >= highest:
                highest = count[option]
                voted = option

        classification.append(voted) 
               
    return classification
    

def evaluateForest(forest, data, target):
    classification = vote(forest, data)
    n_mistakes = mistakes(classification, data, target)
    
    return 1 - float(n_mistakes)/len(classification)
